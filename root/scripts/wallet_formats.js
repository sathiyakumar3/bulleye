...
function icon_nd_name_nd_description(icon, name, description) { var html_div = '<div class="d-flex align-items-center">' + '<div class="symbol symbol-50 symbol-light mr-4">' + '<span class="symbol-label">' + '<span class="svg-icon svg-icon-2x svg-icon-success">' + '<svg><use xlink:href="#' + icon + '"></use></svg>' + '</span>' + '</span>' + '</div>' + '<div class="ml-2">' + '<div class="text-dark-75 font-weight-bold line-height-sm">' + name + '</div>' + '<a href="#" class="font-size-sm text-dark-50 text-hover-primary">' + description + '</a>' + '</div></div>'; return html_div; }
...
function percentage_form(value, total, item) {
    if (value == 0) { var pecentage = 0; } else { var pecentage = Math.round((value / total) * 100); }
...
function swalfire(i, num_of_repeat) {
    if (Swal.isVisible()) { document.getElementById('progres_par').innerHTML = percentage_form(i, num_of_repeat - 1, 'Items'); } else { Swal.fire({ title: 'Please Wait', html: '<div id=\"progres_par\" >' + percentage_form(0, 0, 'Items') + '</div>', showConfirmButton: false, }); }
    if (i == (num_of_repeat - 1)) { Swal.fire({ showConfirmButton: false, }).close(); }
}