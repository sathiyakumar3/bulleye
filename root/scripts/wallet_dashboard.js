...
function build_table(user, obj) {
    ...
    switch (obj[user]['Payment']) {
        case 'Paid':
            var myvar2 = '<a href="javascript:;" class="btn btn-icon btn-light btn-hover-primary btn-sm" onclick="update_entry(\'' + obj[user]['Description'] + '\', \'' + obj[user]['Category'] + '\', \'' + obj[user]['Amount'] + '\', \'' + obj[user]['Timestamp'] + '\', \'' + obj[user]['Type'] + '\', \'' + 'Not Paid' + '\', \'' + obj[user]['user'] + '\', \'' + obj[user]['Repeated'] + '\');update_tabler(\'' + obj[user]['Timestamp'] + '\', \'' + 'Not Paid' + '\')">' + '<span class="svg-icon svg-icon-primary svg-icon-2x"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\themes\metronic\theme\html\demo1\dist/../src/media/svg/icons\Code\Error-circle.svg--> <svg><use xlink:href="#cross"></use></svg></span>' + '</a>';
            break;
        case 'Not Paid':
            var myvar2 = '<a href="javascript:;" class="btn btn-icon btn-light btn-hover-primary btn-sm" onclick="update_entry(\'' + obj[user]['Description'] + '\', \'' + obj[user]['Category'] + '\', \'' + obj[user]['Amount'] + '\', \'' + obj[user]['Timestamp'] + '\', \'' + obj[user]['Type'] + '\', \'' + 'Paid' + '\', \'' + obj[user]['user'] + '\', \'' + obj[user]['Repeated'] + '\');update_tabler(\'' + obj[user]['Timestamp'] + '\', \'' + 'Paid' + '\')">' + '<span class="svg-icon svg-icon-primary svg-icon-2x"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\themes\metronic\theme\html\demo1\dist/../src/media/svg/icons\Code\Done-circle.svg--> <svg><use xlink:href="#tick"></use></svg></span>' + '</a>';
            break;
        default:
    }
    var myvar = '<tr>' + '<td class="pl-0 py-8">' + '<div class="d-flex align-items-center">' +
        icon_nd_name_nd_description(get_cat_ic(obj[user]['Category']), obj[user]['Category'], obj[user]['Description']) + '</div>' + '</td>' + '<td>' + dnt4table(obj[user]['Timestamp']) + '</td>' + '<td>' + payment_status_fomt(obj[user]['Type'], obj[user]['Payment'], obj[user]['Amount'], 'Income') + '</td>' + '<td>' + payment_status_fomt(obj[user]['Type'], obj[user]['Payment'], obj[user]['Amount'], 'Expense') + '</td>' + '<td class="text-centre">' +
        myvar2 + '</td>' + '</tr>';
    return myvar
}
...
function remove_not_paid(array) {
    var obj = [];
    Object.keys(array).sort().map(function(key, index) { if (array[key]['Payment'] == 'Not Paid') { obj.push(array[key]); }; });
    return obj;
}

function update_tabler(timestamp, payment) {
    Object.keys(tabler).sort().map(function(key, index) { var ts = new Date(tabler[key]['Timestamp']); var ts2 = new Date(timestamp); if (ts - ts2 == 0) { tabler[key]['Payment'] = payment; } });
...
var start_app = function() {
    var run_dashboard = function(first_time, tabler) {
        ...
        process_row(tabler, "table_23", '', '', 3);
        if (first_time) { _initDaterangepicker(first_day, last_day); }
    }
    var read_data = function(from, to, first_time, force) { get_wallet_data(wallet_id, from, to, force).then(function(finalResult) { run_dashboard(first_time, finalResult); }).catch((error) => { console.log(error); }); };
